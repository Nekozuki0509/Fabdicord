...
public class Log implements Runnable {
    ...
    @Override
    public void run() {
        final File file = new File("%s/logs/latest.log".formatted(Common.getApi().getGameDir().toString()));
        ...
        while (true) {
            ...
            try (InputStream is = Files.newInputStream(file.toPath(), StandardOpenOption.READ)) {
                ...
                Common.getLOGGER().error("Error while reading Minecraft log file: {}", ExceptionUtils.getStackTrace(e));
    ...
    private void sendLogChannelMessage(String message) {
        ...
        if (MINECRAFT_SEND_COUNT <= 20) {
            Discord.getLogChannel().sendMessage(message).queue();
        }
    }
}